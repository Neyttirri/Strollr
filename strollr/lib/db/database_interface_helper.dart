...
// *************************  Statistics - Duration *************************
/*   
    static Future<YearWithDistances> getAllMonthlyDurationsInYear(
      int year) async {
    List<YearlyDuration> monthsWithWalks =
        await DatabaseManager.instance.readAllWalkDurationsYearly();
    YearWithDistances yearDur = YearWithDistances(year);
    for (MonthlyDuration monthly in monthsWithWalks) {
      yearDur.distancesPerMonth[monthly.monthInYear - 1] = monthly.distance;
    }
    return yearDur;
  } */

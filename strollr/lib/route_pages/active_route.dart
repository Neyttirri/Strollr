...
class _ActiveRouteState extends State<ActiveRoute> {
  ...
  * updates covered distance
   */
   Timer startTracking(){
     return Timer.periodic(_trackingInterval, (timer) async {
       ...
       if (gpx.wpts.length != 0 && gpx.wpts[gpx.wpts.length - 1].lat == currentPosition.latitude
           && gpx.wpts[gpx.wpts.length - 1].lon == currentPosition.longitude) return;

       double lat1 = gpx.wpts.length == 0 ? currentPosition.latitude : gpx.wpts[gpx.wpts.length - 1].lat;
       double lat2 = currentPosition.latitude;

       double lon1 = gpx.wpts.length == 0 ? currentPosition.longitude : gpx.wpts[gpx.wpts.length - 1].lon;
  ...
  double calcDistance(double lat1, double lat2, double lon1, double lon2){
    ...
    double a = pow(sin(distLat / 2), 2) + cos(rLat1) * cos(rLat2) * pow(sin(distLon / 2), 2); //some weird math hyper brain stuff
